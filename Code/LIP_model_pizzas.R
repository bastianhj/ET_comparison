...
for (case in cases){
  ...
  data=data%>%group_by(pid)%>%mutate(max_freq=max(sum(Choice_id==1),
                                                  sum(Choice_id==2)))%>%filter(max_freq<12)
...
#   data=data%>%filter(RT>0)
  ...
  data=data%>%
    mutate(
      Qual_FT_sum_norm=((Qual_FT_count)/Tot_counts),#/sum(FT_sum)
      Cost_FT_sum_norm=((Cost_FT_count)/Tot_counts))

  temp=data %>%
    filter(Trial == 1) %>% select(Cost_FT_count,Qual_FT_count,pid,
                                  Cost_FT_time,Qual_FT_time,Tot_counts)
  temp$Qual_FT_norm = normalize(temp$Qual_FT_count)#/temp$Tot_counts
 ...
 # temp$Travel_FT_norm = normalize(temp$Travel_FT_count)#/temp$Tot_counts
  ...
  apollo_fixed=c('sigma_IP','b_pizza1','gamma0_cost')
  ...
  apollo_randCoeff=function(apollo_beta, apollo_inputs){
    ...
    randcoeff[['b_qual_LV']]=b_qual+alpha_qual*randcoeff[["IP"]]
  ...
  apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
    ...
    V=list()
    V[['pizza1']]=b_pizza1+(b_cost_LV)*cost1+(b_qual_LV)*qual1
    
    V[['pizza2']]=b_pizza2+(b_cost_LV)*cost2+(b_qual_LV)*qual2
    
    mnl_settings = list(
      utilities=V,
      alternatives  = c(pizza1=1,pizza2=2),
      avail=1,
      choiceVar = Choice_id
    )
    ...
    normalDensity_settings1 = list(outcomeNormal = (Cost_FT_norm), #valor real (observado)
                                   ...
                                   rows          = (Trial==1),
                                   componentName = "cost")
    normalDensity_settings2 = list(outcomeNormal = (Qual_FT_norm),
                                   xNormal       = IP*gamma_qual,
                                   mu            = gamma0_qual,
                                   sigma         = abs(sigma_qual),
                                   rows          = (Trial==1),
                                   componentName = "qual")
    
    P[["cost"]]     = apollo_normalDensity(normalDensity_settings1, functionality)
    P[["qual"]] = apollo_normalDensity(normalDensity_settings2, functionality)
...
b_qual_post=cond$b_qual_LV
IP_temp=cond$IP
temp=cbind(exp=exp,device=case,ET=ET,IP_temp,
           IP_alpha_qual=model$estimate['alpha_qual']*IP_temp$post.mean,
           IP_alpha_cost=model$estimate['alpha_cost']*IP_temp$post.mean)
IPs=rbind(IPs,temp)
## calculate QST
QST=(b_qual_post$post.mean/b_cost_post$post.mean)

QST=data.frame(QST=QST,exp=exp,device=case,ET=ET)
QST_summary=rbind(QST_summary,QST)
#alpha/beta
delta_settings=list(expression=c('alpha_qual/b_qual',
                                 'alpha_cost/b_cost'))

temp=apollo_deltaMethod(model,deltaMethod_settings = delta_settings)
temp=cbind(temp,alpha_mean=c(model$estimate['alpha_qual'],
                        model$estimate['alpha_cost']),
           alpha_se=c(model$robse['alpha_qual'],
...
#   QST=(b_time_post$post.mean/b_cost_post$post.mean)
# 
#   QST=data.frame(QST=QST,exp=exp,device=case,ET=ET)
#   QST_summary=rbind(QST_summary,QST)
...
rm(ET)
QST_summary$QST=-QST_summary$QST

QST_summary$ET[QST_summary$ET=='WG']='Web-Gazer'
QST_summary$ET[QST_summary$ET=='WL']='Eye-Link'
QST_summary<-QST_summary%>%filter(ET=='Eye-Link'|ET=='Web-Gazer')

QST_summary=QST_summary%>%group_by(device,ET)%>%mutate(mean_QST=mean(QST),
                                                               sd=sd(QST))

## QST distribution
QST_kstest <- QST_summary %>%
  group_by(device) %>%
  do({
    data = .
    combn(unique(data$ET), 2, function(x) {
      test <- ks.test(data$QST[data$ET == x[1]], data$QST[data$ET == x[2]])
      t_test<- t.test(data$QST[data$ET == x[1]], data$QST[data$ET == x[2]])
      mean_qst1<-mean(data$QST[data$ET == x[1]])
      mean_qst2<-mean(data$QST[data$ET == x[2]])
      se_qst1<-sd(data$QST[data$ET == x[1]])/sqrt(sum(data$ET == x[1]))
      se_qst2<-sd(data$QST[data$ET == x[2]])/sqrt(sum(data$ET == x[2]))
      ...
      overlap<-overlap(list(data$QST[data$ET == x[1]], data$QST[data$ET == x[2]]))
...
write.csv(QST_kstest,'output/QST_tests.csv')

desired_order2 <- c("monitor", "laptop", "tablet","mobile")  # Adjust as needed

# Reorder the 'attribute' within the ggplot call or before it
QST_kstest$Comparison=as.factor(QST_kstest$Comparison)
QST_kstest$Comparison=as.numeric(QST_kstest$Comparison)
QST_kstest$device<-factor(QST_kstest$device,levels=desired_order2)
QST_summary$device<-factor(QST_summary$device,levels=desired_order2)

# Define a color palette
palette <- brewer.pal(n = 4, name = "Dark2")


p1<-ggplot(QST_summary, aes(x=QST,fill=ET,color=ET)) +
  geom_density(alpha=0.3,position='identity') +
  geom_vline(aes(xintercept = mean_QST,color=ET), show.legend = FALSE) +
  ...
  # geom_text(data=QST_kstest, aes(label=Label_ov, y=Inf, x=Inf,color=NULL,fill=NULL),
  #           vjust=QST_kstest$Comparison+(QST_kstest$Comparison-1)*1.5, hjust=1, size=2.5, color="black",
  #           show.legend = FALSE,
  #           show_guide  = FALSE)  +
  theme(legend.position = "bottom") 


QST_kstest <- QST_kstest %>%
  arrange(overlap,device) %>%
  mutate(Group = factor(paste0(Group1, "-", Group2), levels = unique(paste0(Group1, "-", Group2))))


p2 <- ggplot(QST_kstest,aes(x=device,y=overlap,fill=device))+
...
png(filename="QWTP_v1.png",width=20,height=15,units="cm",res=300)
print(combined_plot_horizontal)
dev.off()


## QST - across devices
QST_kstest2 <- QST_summary %>%
  group_by(ET) %>%
  do({
    data = .
    combn(unique(data$device), 2, function(x) {
      test <- ks.test(data$QST[data$device == x[1]], data$QST[data$device == x[2]])
      overlap<-overlap(list(data$QST[data$device == x[1]], data$QST[data$device == x[2]]))
      data.frame(Group1 = x[1], Group2 = x[2], D.statistic = test$statistic, P.value = test$p.value,
                 overlap=overlap$OV
      )
    }, simplify = FALSE) %>% bind_rows()
...
QST_kstest2$Comparison=as.factor(QST_kstest2$Comparison)
QST_kstest2$Comparison=as.numeric(QST_kstest2$Comparison)

p1<-ggplot(QST_summary, aes(x=QST,fill=device,color=device)) +
  geom_density(alpha=0.4,position='identity') +
  #geom_vline(aes(xintercept = mean_QST,color=device), show.legend = FALSE) +
  facet_wrap(~ET, ncol=2,scales="free_y") +
  scale_fill_manual(values = palette) +  # Apply the custom color palette
  scale_color_manual(values = palette) +# Define a color palette
  theme_minimal() +
  xlab('QWTP [SGD/qual]') + ylab('density') +
  guides(fill=guide_legend(title="Device"), color="none") +
  # geom_text(data=QST_kstest2, aes(label=Label_ov, y=Inf, x=Inf,color=NULL,fill=NULL),
  #           vjust=QST_kstest2$Comparison+(QST_kstest2$Comparison-1)*1.5, hjust=1, size=2.5, color="black",
  #           show.legend = FALSE,
  #           show_guide  = FALSE)  +
  theme(legend.position = "bottom") 


QST_kstest2 <- QST_kstest2 %>%
  arrange(ET, overlap) %>%
  mutate(Group = factor(paste0(Group1, "-", Group2), levels = unique(paste0(Group1, "-", Group2))))


p2 <- ggplot(QST_kstest2,aes(x=Group,y=overlap,fill=ET))+
  geom_bar(alpha=0.8,stat="identity",position="dodge")+
...
# Print one of the combined layouts
png(filename="QWTP_v2.png",width=20,height=15,units="cm",res=300)
print(combined_plot_horizontal)
dev.off()
...
write.csv(Summary_kstest,'output/IP_test.csv')
...
p1<-ggplot(summary_data, aes(x=mean_prob, fill=ET, color=ET)) +
  ...
  facet_wrap(~device+Variable, ncol=2,scales="free_y") +
  theme_minimal() +
...
# Print one of the combined layouts
png(filename="IP_alpha_v1.png",width=20,height=15,units="cm",res=300)
print(combined_plot_horizontal)
dev.off()
...
p<-ggplot(summary_data, aes(x=mean_prob, fill=device, color=device)) +
  ...
  scale_fill_manual(values = palette) +  
  scale_color_manual(values = palette) +
  facet_wrap(~ET+Variable, ncol=2,scales="free") +
  theme_minimal() +
  xlab(TeX("Posterior mean $IP_n\\cdot \\alpha_k$")) +
  ylab('Density') +theme(legend.position = "bottom")
...
# Print one of the combined layouts
png(filename="IP_alpha_v2.png",width=20,height=15,units="cm",res=300)
print(combined_plot_horizontal)
dev.off()
